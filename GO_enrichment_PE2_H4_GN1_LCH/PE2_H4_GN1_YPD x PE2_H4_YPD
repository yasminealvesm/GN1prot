...
filtered_data = data[(data['Comparison (condition 1 / condition 2)'] == 'Test 1 / Ctrl-1') & (data['Pvalue'] < 0.05)]
...
fig, ax = plt.subplots(figsize=(12, 8))  # conform needs
...
for namespace, color in category_colors.items():
    ...
    bars = ax.barh(subset['GO Name'] + ' (' + subset.index.get_level_values('GO Id').astype(str) + ')',
                   ...
                   color=color,
                   label=namespace)  # labels in bars
...
ax.set_title('Top Enriched GO Terms for PE2_H4_GN1_YPD / PE2_H4_YPD')
